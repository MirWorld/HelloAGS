...
**路径约定（避免歧义）:** 本文件中 `plan/` 代表项目根目录下的 `helloagents/plan/`（同理 `history/` → `helloagents/history/`），与 G11 保持一致。
...
在方案设计阶段（规划域）：
- ✅ 允许：只读扫描/读取项目文件，形成方案；写入 `helloagents/` 工作区（`plan/`、`project.md`、`active_context.md`、`wiki/` 等）
- ✅ 允许：运行**只读命令**辅助定位与取证（定义见 `references/command-policy.md`）
- 🚫 禁止：修改业务代码/配置（`helloagents/` 工作区之外的文件）
- 🚫 禁止：运行**有副作用命令**（定义见 `references/command-policy.md`）；如确需验证，留到开发实施阶段执行

补充（写入范围 `write_scope`，见 `references/routing.md`）：
- 若 `write_scope = no_write`：本阶段不得写入 `helloagents/` 工作区；只在对话中给方案/清单（必要时进入等待态）
- 若用户希望“方案落盘以便续作/执行”：将写入范围提升为 `helloagents_only`（只写 `helloagents/`，不改业务代码）
...
- 规则：
  - 优先复用 `helloagents/project.md#项目能力画像` 中已有命令矩阵
  - 不足则探测并补齐（方案设计允许写入 `helloagents/project.md`）
...
- 规则：
  - `helloagents/active_context.md` 不存在 → 使用 `../templates/active-context-template.md` 创建（允许写入）
...
- `task.md` 必须包含：
  - 对齐确认（引用 `why.md#对齐摘要`）
  - 项目能力画像确认（引用 `helloagents/project.md#项目能力画像`）
  - 复用检索与边界确认
  - （如触发）跨层一致性检查：按 `../references/cross-layer-checklist.md` 清点受影响层/契约/消费者，并把结论落到 how.md 或 task.md
  - active_context（可验证接口注册表）规则：
      - 执行前阅读 `helloagents/active_context.md`（只信 `[SRC:CODE]` 条目）
      - 如本次变更影响公共接口/契约/数据流：必须更新 `helloagents/active_context.md` 并补齐 `[SRC:CODE]` 指针
  - 执行期护栏（越界保护）：
