...
**默认倾向（减负，不降标准）:** 对明确的小改动，先尝试 Quick Fix/轻量迭代；不要因保守直接拉到标准开发。Quick Fix 门槛本身不放宽，但命中后要优先使用。
...
- Quick Fix（≤2文件≤30行、无 EHRB、无架构影响）：
  ...
  - 命中时默认直接走 Quick Fix；不要因“先完整一点更稳/顺手做大一点”自动升级
  - 仅当命中升级信号时升级：预计 `>2 文件` / `>30 行`、`verify_min` 不明确、真值源/消费者/单位边界无法一次收敛、涉及契约/DB/鉴权/依赖/架构/EHRB、改动范围不确定
...
交互确认模式:
  ...
  - 停止并等待用户明确确认
  - 用户响应处理：
    - 明确确认("是"/"确认"/"可以"/"按方案做"/"开始实现"/"继续执行"等纯确认或执行表述) → 进入开发实施
    - 带疑问词或问号的输入（如“好，那有没有更强的？”“为什么这么做？”）→ 先按咨询/追问回答，不视为确认，不进入执行
    - 明确拒绝("否"/"取消"等) → 流程终止
