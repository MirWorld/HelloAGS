...
**目标:** 按方案包中任务清单执行代码改动，同步更新知识库；仅当归档就绪门禁通过时迁移到 history/
...
**运行时守卫前移（如已接 hooks）:**  
- `SessionStart`：预检 `_current.md` 指针是否合法  
- `UserPromptSubmit`：优先处理 Pending、当前包合法性、功能删减等待态  
- `Stop`：把 `model_event` / 恢复检查点尽量无感回填到 `task.md##上下文快照`  
- `PreCompact/PostCompact`：压缩前后写入 `compact_event`；`post_compact` 是恢复硬门触发信号
- `PreToolUse`：若 `post_compact` 未恢复，工具级阻断业务文件读取、代码修改、验证命令与临时重规划，只允许 Hydration 白名单
- 主流程只消费这些守卫产出的**磁盘事实**（`_current.md`、`task.md##上下文快照`、`capture-runtime-events.ps1` 的结果），除非 hooks 缺失或失效，不重复做口头解释  
- 若 hooks 已产出 `Pending` / `current_package` / `model_event` / `next_unique_action` 等结构事实：主流程默认直接消费，不再重复做同类人工判断
- 命中 `model_event: model_rerouted | response_incomplete` 时，恢复检查点只说明“可以恢复”；在继续改代码前仍必须复核当前 contract 是否仍成立（目标/成功标准/非目标/禁止项）
- 命中 `compact_event: post_compact` 或 `signal: compact_resume_required` 时，必须先通过 `references/resume-protocol.md` 的 Resume Hydration Gate；Hydration 完成前禁止读取业务文件继续实现、禁止临时重规划、禁止凭聊天摘要判断进度
- hooks 仍是增强层；真值仍以方案包落盘为准
...
- 如存在 `HAGSWorks/scripts/capture-runtime-events.ps1`：在任何 `-Mode exec` 校验/执行前，先**最佳努力**运行一次用于回填 `model_event`（例如 `model/rerouted`、`response.incomplete`）到 `task.md##上下文快照`，并自动追加恢复检查点（降低压缩/断层续作误重做概率）；脚本不存在或回填失败不阻断（无感增强）。
  - 若 Stop hook 已提前回填：脚本应保持幂等；主流程只消费 `task.md` 中已有的 `model_event` / 恢复检查点，不重复解释事件本身。
...
### 步骤1.1: Resume Hydration Gate（压缩后硬停）
...
- 若已接 `scripts/hooks/helloagents-pretooluse.ps1`：同一 assistant turn 内发生压缩后，后续工具调用也必须执行上述白名单；不得等待下一次 `UserPromptSubmit` 才恢复。
...
- 若 `HAGSWorks/project.md` 已定义“架构不变量”：执行时必须将其视为项目级上位约束，而不是只参考当前局部代码形态
...
- 关键节点微计划：首次改代码前、Patch/验证失败后、进入验证前、Review 发现问题后、准备扩大范围或触碰已完成任务前，只做 1–3 行微计划（当前任务 / 下一步唯一动作 / 验证），不要把普通小任务展开成重型流程。
...
- 检查 `how.md` 是否包含结构质量约束（边界/复用/重构预算/质量门禁）、`carry_forward_verify` 与“相邻变化压力”（命中时），并在执行中遵守
- 如发现对齐摘要与任务/实现路径明显冲突：
...
- 长任务（多任务 / 多轮 / 重复触碰同一 Workset）默认维护 `progress_phase: start|mid|late|final`；至少在中程写 1 次 `mid`，Review 前更新到 `late` 或 `final`
...
### 步骤4.5: 防重复执行（No-Redo，硬规则）

<!-- CONTRACT: develop-no-redo v1 -->

目标：在会话压缩/断层续作后，避免“已完成任务被再次提起并重复执行”导致二次修改与引入新错误。

规则（命中即执行）：
- 若 `task.md` 中所有任务均为 `[√]` 或 `[-]`，且 `### 待用户输入（Pending）` 为空：判定“执行任务可能已完成”；禁止继续改代码；下一步只允许进入 Archive Readiness Gate。门禁未通过时，保持方案包在 `HAGSWorks/plan/` 并继续使用 `_current.md` 续作，不得临时重规划。
- 若必须触碰“已完成任务”涉及的文件/逻辑（例如修正遗漏/新约束/回归失败）：
  1. **先写 Delta**：在 `task.md##上下文快照` 的“决策”区追加一条 Delta（含来源标签 `[SRC:USER|CODE|TOOL]`、原因、影响、下一步唯一动作）
  2. **再新增任务承接**：新增 1 条新的 `- [ ]` 任务（明确文件/范围/verify），用于承接该 Delta
  3. **禁止无说明重做**：禁止在原 `[√]` 任务下悄悄改动，避免续作时“看起来已完成但事实已变”
...
执行规则:
  - 严格按 task.md 逐项执行
  - 每完成一项并准备进入下一项前，先同步 task.md 状态；若下一项的执行域或验证发生变化，按微计划格式更新“下一步唯一动作”
  - Quick Fix 只维护 1–3 条任务；满足快路径条件时不复制标准方案包长清单

任务成功处理:
  - 每个任务执行成功后，只有在本任务对应的 `verify_min` 与已触发的 build/test 门禁通过（或明确 `[-]` 并写明原因）后，才将状态从 [ ] 更新为 [√]
  - 若本任务再次触碰同一 Workset，或 `how.md` 已列出 `carry_forward_verify`：在标记 [√] 前，必须补跑这些既有验证或明确记录“本轮不受影响”
  ...
  - 若任务完成但暂不运行更重门禁，必须记录原因与已执行的最小验证，避免“看起来完成但证据缺失”
...
- 没有显性失败，但出现“重复修改 / 无新增证据 / 下一步动作不变”的隐性空转
...
两段式 Review:
  ...
  2) 结构与质量:
      ...
      - 是否新增重复逻辑或 utils 膨胀
      - 命名/抽象是否可读且贴近领域语义
      - 长任务是否出现“结构侵蚀”：热点文件持续膨胀、临时胶水增加、后置任务打穿前置成功标准
      - `how.md` 已定义的 `carry_forward_verify` / “相邻变化压力” 是否已消费，而不是只完成当前一步
...
Review 记录:
  ...
  - 默认优先记录：遗漏 / 缩水 / 越界 / 未显式返工 / contract 偏移 / 结构侵蚀 / 累计回归 / 设计债务，而不是只复述“做了什么”
```

### 步骤8: 同步更新知识库

**重要:** 必须在步骤12执行 Archive Readiness Gate / 迁移方案包前完成方案包内容读取
...
### 步骤12: Archive Readiness Gate 与完成态迁移

<plan_migration>

⚠️ **CRITICAL - 防早归档规则:**

**本轮执行结束不等于方案完成。** 只有归档就绪门禁通过，才允许迁移到 `history/`。
...
**警告:** 迁移操作将导致 plan/ 下的源文件路径失效；门禁未通过时必须保留 plan/ 源目录，避免长任务续作丢失。
</plan_migration>
...
**规范要求:**
- **注释与文档风格:** 优先遵循项目既有约定（语言/格式/TSDoc/GoDoc/Rustdoc/...）；若项目无明确约定，默认使用简体中文（遵循 G1）
...
[1] YYYYMMDDHHMM_<feature2> - [概要描述]（current）
[2] YYYYMMDDHHMM_<feature1> - [概要描述]
...
2. **阶段具体内容(≤5条要点):**
   ...
   - 🔍 质量验证: 一致性审计、测试结果
   - 💡 代码质量优化建议（如有）
   - 📦 归档状态: 已迁移至 `history/YYYY-MM/YYYYMMDDHHMM_<feature>/`，或门禁未通过并保持 `HAGSWorks/plan/<package>/` active
3. **文件变更清单:**
   ```
   📁 变更:
     - {代码文件}
     ...
     - HAGSWorks/history/index.md（仅归档发生时）
     ...
   ```
...
完成所有动作后:
  ...
  全授权命令: 输出整体总结 → 流程结束
  执行命令: 输出整体总结 → 流程结束
  变量清理: 仅在 Archive Readiness Gate 通过并完成迁移后清理 CURRENT_PACKAGE / `_current.md`；门禁未通过时保持 active
