...
1. 按 `references/quickfix-protocol.md` 创建最小完整方案包：
   - `HAGSWorks/plan/YYYYMMDDHHMM_quickfix_auth-timeout/why.md`
   - `HAGSWorks/plan/YYYYMMDDHHMM_quickfix_auth-timeout/how.md`
   - `HAGSWorks/plan/YYYYMMDDHHMM_quickfix_auth-timeout/task.md`
2. 执行“改一个参数微清单”（真值源/单位边界/消费者/文档/最小验证），把结论写入 `task.md##上下文快照`
3. 写执行域声明（Allow/Deny/NewFiles/Refactor=否），并把“下一步唯一动作”落盘
4. 只改必要代码 + 运行最小验证
5. 迁移方案包到 `HAGSWorks/history/YYYY-MM/...`
