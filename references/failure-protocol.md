...
调试底线：
- 若没有最小复现方式、可观察反馈信号或可执行验证动作，禁止继续盲改；先补证据、缩小复现场景，或向用户索取复现条件。
- 连续修改同一 Workset 但没有新增验证输出 / 错误输出 / 观察证据时，不一定计入显性失败，但应视为隐性空转风险，提前触发 `references/break-loop-checklist.md`。
- 每次“猜测性修复”之前必须写清本轮要验证的主假设；同一假设失败后不得换写法继续赌运气，应换假设、缩范围或升级决策。
- **Fail loud**：失败、跳过、未验证、hook `SKIP`、脚本异常、验证未覆盖成功标准，都必须显式暴露；不得把“不阻断”写成“通过”，不得把“未执行”写成“完成”。
...
补充边界：
- `连续失败` 是**显性失败**计数（有阻断性错误 / 明确无法继续）
- `隐性空转`（无显性失败，但反复修改/无新增证据/下一步动作不变）即使未达到失败阈值，也可以提前触发 `references/break-loop-checklist.md`
...
失败显式化最小记录（Fail loud）：
- [SRC:TOOL] failed_or_skipped: `<失败/跳过/未验证项>` reason: `<原因>`
- [SRC:TOOL|INFER] impact: `<阻断/警告/未知>` coverage: `<覆盖了什么/没覆盖什么>`
- 下一步唯一动作: `<补验证/补复现/用户决策/降级路径>` 预期: 获得可继续判断的证据

调试反馈循环记录格式（推荐，结构化，不绑文案）：
- [SRC:TOOL] reproduce: `<命令/步骤>` actual: `<错误/异常摘要>`
- [SRC:INFER] hypothesis: `<本轮只验证一个主要假设>`
- [SRC:CODE] change_scope: `<最小改动面>`
- [SRC:TOOL] feedback_command: `<修改后要运行的最小反馈命令/步骤>`
- 下一步唯一动作: `运行 feedback_command 或补复现条件` 预期: 获得新证据后再决定是否继续修改
