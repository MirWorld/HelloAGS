...
3. **运行时异常信号未闭环**（模型切换/输出不完整/降级提示/压缩恢复未完成）→ 先落盘证据与恢复检查点，再继续
   - 触发信号（启发式，避免绑定具体措辞；命中任一即可）：  
     - 看到 `model/rerouted`（或 “routed … fallback/降级/以不同模型继续” 类系统提示）  
     - 看到 `response.incomplete` / “输出不完整/被截断” 类提示  
     - 当前方案包存在未恢复的 `compact_event: post_compact`，或 hooks 注入 `signal: compact_resume_required`
   - 处理原则：把信号当作 `[SRC:TOOL]`，并结构化落到 `task.md##上下文快照`：  
     ...
     - `compact_event: post_compact` 必须按 `references/resume-protocol.md` 的 Resume Hydration Gate 写回 `reboot_check` / `contract_checkpoint`
     - 在其后追加**恢复检查点**（至少包含 `repo_state:` 与 `下一步唯一动作:`；细则见 `references/context-snapshot.md`）  
   - 路由：优先改走 `references/resume-protocol.md` 的断层恢复（禁止依赖聊天记忆继续推进）；纯续作输入只允许进入 `recovery_only` / `hydration_only`，显式执行命令（如 `~exec/~auto`）必须先完成恢复；若 `write_scope = no_write` 则只做只读恢复与对话内说明（不得落盘）。
...
4. **咨询问答（纯问句优先）**：解释/讨论/比较/选型/反问/追问原因，且没有显式执行动词 → `write_scope = no_write`，不得创建/推进方案包
   - 若任务为“提示词/规则优化”：按 `references/prompt-optimization.md` 的诊断清单与三档交付输出；默认 `write_scope = no_write`
5. **开发模式**：先判微调（Quick Fix，命中即停）→ 不命中再轻量迭代 → 标准开发 → 完整研发（兜底）
...
- 用户输入是纯问句/追问/选型比较（如“有没有更强的控件”“为什么选这个”“区别是什么”“哪个更好”“是否有替代”），且没有明确执行动词 → `no_write`；即使当前存在 active package，也只把它当背景，不得把问句升级为方案设计/执行
...
### 4.0 方案包创建门槛（防旧任务劫持新问题）

创建或推进方案包必须先满足**显式写入/执行意图**，不得只因存在 active package 就进入开发模式：

- 允许进入方案包流程：`~plan`、`~auto`、`~exec`、`实现/修复/改/落地/按这个做/按方案做/生成方案并落盘/开始实现`
- 禁止创建或推进方案包：纯问句、选型比较、追问原因、反驳方案、要求解释、搜索资料后要求结论
- active package 只在以下情况下成为主任务状态源：用户明确“继续/接着/上次”、显式执行命令、或明确要求按当前方案实施
- 若本轮为回答问题而执行了搜索/外部知识读取，输出必须先回答搜索目标；不得搜索完直接跳回“方案设计完成/下一步确认”

示例：
- `那没有比这性能强的控件了？` → 咨询问答，回答控件选型，不创建 `HAGSWorks/plan`
- `为什么选 Virtual TreeView？` → 咨询问答，解释原因与边界
- `好，按这个方案实现` → 开发/执行路径
- `继续` → 续作/断层恢复路径
...
- 升级规则：只有在执行前或取证中命中“升级信号”时，才升级到 4.2/4.3/4.4；否则保持 Quick Fix，不因保守倾向自动加重流程。
