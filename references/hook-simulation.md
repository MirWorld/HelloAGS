...
目标：把 Claude Code 的 “Hooks + Skills + Commands + Agents” 里最有确定收益的部分，**在不把 hooks 当作核心前提**的前提下，落到 helloagents 的“阻断式路由 + 触发器清单 + 触发式落盘 + 收尾 Review”里；当 Codex hooks 可用时，再用最薄桥接脚本增强稳定性。

结论先说清：
- Claude Code 的 `.claude/hooks/*` 属于 Claude 工具链的生命周期钩子机制；
- **Codex CLI 已提供 hooks engine 的版本可以接入本仓库脚本；当前 skill 包不把 hooks 当作核心前提**；
- **hooks 不是 helloagents 自带能力**；它们是 Codex 提供时可选接入的外部能力，本仓库只负责提供桥接脚本与模板；
- **一旦接了 hooks，就应该让它们承担重复性守卫**：Pending 阻断、`_current.md` 预检、功能删减等待态提示、`model_event` 回填、`PreCompact/PostCompact` 快照写入，以及压缩后未 Hydration 的 `PreToolUse` 工具级硬门；主流程只消费结果，不重复解释；
...
3. **收尾复盘（Stop Hook）**：任务结束时强制总结变更与下一步，并建议独立审查/验证；Stop 只做收尾与提示，归档必须另过 Archive Readiness Gate
...
|---|---|---|---|---|
| SessionStart（会话启动） | 若是续作/不确定，或检测到 `post_compact` / `compact_resume_required` / `model/rerouted` / `response.incomplete`：按恢复协议重建进度；否则只做最小环境确认 | `references/resume-protocol.md`、`references/read-paths.md` | `task.md##上下文快照`：Workset + 下一步唯一动作（必要时） | 3 分钟内恢复到“下一步唯一动作” |
| UserPromptSubmit（用户提交问题） | **先路由**：命令/等待回复/只给方案/执行缺方案包/EHRB → 命中即阻断；若是纯续作且存在未恢复断层，只注入 `recovery_only` / `hydration_only` | `references/routing.md`、`templates/output-format.md` | 需要等待用户时：写 `Pending` + 输出末尾 `<helloagents_state>`；恢复态写回 `repo_state` / `reboot_check` / `contract_checkpoint` | 路由明确且下一步唯一动作清晰 |
| PreToolUse（执行工具前） | 规划域只读；执行域先写执行域声明；若存在未恢复 `post_compact`，只允许 Hydration 白名单工具，阻断业务文件/写代码/验证命令 | `references/command-policy.md`、`references/execution-guard.md`、`references/resume-protocol.md` | `task.md##上下文快照`：决策（Allow/Deny/NewFiles/Refactor）+ 下一步唯一动作；Hydration 成功后写 `reboot_check` / `contract_checkpoint` | 边界清晰、命令分级清晰、可验证动作已绑定；Hydration 完成前不得继续实现 |
| Stop（任务结束） | 收尾清单 + 两段式 Review + 统一输出；只提示是否具备归档条件，不直接迁移 history | `references/finish-checklist.md`、`references/review-protocol.md`、`references/plan-lifecycle.md`、`templates/output-format.md` | `task.md##Review 记录`：门禁证据；`task.md##上下文快照`：最终检查点；归档前需通过 Archive Readiness Gate | 输出包含：改动了什么/在哪里/验证结果；未过门禁时保持方案包 active |
...
- `scripts/hooks/helloagents-sessionstart.ps1`
  - 输入：stdin JSON（或 `-InputFile`）
  - 行为：校验 `HAGSWorks/plan/_current.md` 的 `current_package` 是否仍指向 `HAGSWorks/plan/` 下的完整方案包；若当前包已完成且无 Pending，或存在未恢复的 `response_incomplete` / `compact_event: post_compact`，输出 WARN 提示下一步
...
- `scripts/hooks/helloagents-userpromptsubmit.ps1`
  ...
  - 增强：当检测到未恢复的 `response_incomplete` / `compact_event: post_compact`、已完成方案包被误续作、或结构化字段 `feature_removal_risk: suspected` 且未批准时，把守卫前移到继续执行之前；纯“继续/接着/上次”只注入 `recovery_only` / `hydration_only`，显式 `~exec/~auto` 仍 `decision=block`
  - 约束：只读磁盘事实（`_current.md` / `task.md` / `how.md`），不依赖自然语言措辞；不执行任何动态内容（payload 只当数据）

- `scripts/hooks/helloagents-pretooluse.ps1`
  - 输入：Codex `PreToolUse` payload（stdin JSON 或 `-InputFile`）
  - 行为：若当前方案包存在未恢复 `compact_event: post_compact`，把同轮后续工具调用切到 `hydration_only`
  - 放行：只读 `HAGSWorks/plan/_current.md`、当前包 `why.md/how.md/task.md`、`git status/rev-parse/diff --stat`；以及只写当前 `task.md` 的恢复检查点
  - 阻断：业务文件读取、代码修改、测试/编译/验证命令、临时重规划；输出 `decision: block` + `reason` + `hookSpecificOutput.additionalContext`
  - 价值：补上“压缩发生在同一轮 assistant 中，未经过下一次 UserPromptSubmit/SessionStart”的硬门缺口
  - 约束：payload 只当数据；不执行工具输入；没有 active package 或未命中未恢复 `post_compact` 时静默放行

- `scripts/hooks/helloagents-compact.ps1`
  - 输入：Codex `PreCompact` / `PostCompact` payload（stdin JSON 或 `-InputFile`）
  - 行为：在当前方案包 `task.md##上下文快照` 追加 `compact_event: pre_compact|post_compact`，并记录 `compact_trigger / session_id / turn_id / model / repo_state / 下一步唯一动作`
  - `PostCompact` 额外写入：`resume_hydration_required: yes`、`reboot_check: required`、`hydrated_from_package:`、`hydration_source: _current.md + task.md + repo_state`；这会触发 `signal: compact_resume_required`，要求先执行 Resume Hydration Gate
  - 价值：`PreCompact` 在压缩真正发生前保存任务进度；`PostCompact` 作为压缩后审计与 Reboot Check 触发信号
  - 输出：遵守官方 compact hook 输出 schema（`continue / suppressOutput / systemMessage`），不使用 `hookSpecificOutput`
  - 约束：payload 只当数据；写 `task.md` 前必须加锁、锁内重新读取并去重；失败降级为 `continue: true`，不阻断正常压缩
...
- 本地 dry-run 夹具：
  ...
  - `templates/hooks/pretooluse-hook-fixture.json`
...
- 脚本定位：模板命令默认按 `HELLOAGENTS_SKILL_ROOT` → `$CODEX_HOME/skills/helloagents` 查找本 skill 包脚本；若你没有安装到默认路径，先设置 `HELLOAGENTS_SKILL_ROOT`，不要把命令留成不可达的相对路径
- 接线健康检查：在目标项目运行 `pwsh -NoProfile -File <helloagents>/scripts/check-target-hooks.ps1 -ProjectRoot <目标项目> -SkillRoot <helloagents>`，确认 `hooks`、`.codex/hooks.json`、`SessionStart/UserPromptSubmit/PreToolUse/Stop/PreCompact/PostCompact` 与脚本路径都可解析
- 真实 dry-run：目标项目已有 active 方案包时，可追加 `-DryRun`；脚本会真实执行 `SessionStart/UserPromptSubmit/PreToolUse/Stop/PreCompact/PostCompact` fixture，并要求 `Stop/PreCompact/PostCompact` 不是 `SKIP`，用于确认压缩前后快照链路真的可用

补充（Codex 0.130.0+）：
- 官方已把 `plugin-bundled hooks`、`hook enablement state`、marketplace 安装与 external-agent config import 做得更完整，意味着 hooks 的分发和启用可以更自然地接入本仓库的桥接模板

补充（Codex 0.130.0+）：
- 官方 hooks 已提供 `PreCompact` / `PostCompact`，可以把压缩生命周期事件接到 `scripts/hooks/helloagents-compact.ps1`
- `PreCompact` 解决的是“压缩发生前一刻写恢复检查点”；它不提供“距离阈值还有多少 token”的提前预警
...
- **确定性交给脚本**：路由状态、恢复状态、归档状态、hook payload 解析、锁/去重/结构字段检查优先由脚本或 hooks 判断；模型只消费结构化结果并负责意图建模、风险取舍与证据解读
...
有一类能力不是 `PreCompact` 直接提供的，但仍然很值：**在接近自动压缩阈值前，先把当前方案包的恢复检查点落盘**。
...
注意：
- `PreCompact` 是压缩边界前的官方 hook，适合写“最后一刻快照”
- `helloagents-context-threshold.ps1` 是阈值提前预警入口，适合在还没真正压缩前更早写快照
