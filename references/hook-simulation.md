...
- `scripts/hooks/helloagents-sessionstart.ps1`
  - 输入：stdin JSON（或 `-InputFile`）
  - 行为：校验 `HAGSWorks/plan/_current.md` 的 `current_package` 是否仍指向 `HAGSWorks/plan/` 下的完整方案包
...
- `scripts/hooks/helloagents-userpromptsubmit.ps1`
  ...
  - 增强：仅当当前输入或当前方案包命中**真实功能删减风险信号**（例如等待功能删减确认、Pending 中明确提到功能删减、用户提示词出现删除/移除/禁用旧能力等）且未获 `feature_removal_approved: yes` 批准时，才额外注入一段低噪声 guard context
...
- 接了 hooks，只是让提示/回填/守卫更无感；**不是**把正确性真值从协议层转移到 hooks
