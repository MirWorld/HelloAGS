...
- **一旦接了 hooks，就应该让它们承担重复性守卫**：Pending 阻断、`_current.md` 预检、功能删减等待态提示、`model_event` 回填、`PreCompact/PostCompact` 快照写入，主流程只消费结果，不重复解释；
...
- 接线健康检查：在目标项目运行 `pwsh -NoProfile -File <helloagents>/scripts/check-target-hooks.ps1 -ProjectRoot <目标项目> -SkillRoot <helloagents>`，确认 `codex_hooks`、`.codex/hooks.json`、`SessionStart/UserPromptSubmit/Stop/PreCompact/PostCompact` 与脚本路径都可解析
- 真实 dry-run：目标项目已有 active 方案包时，可追加 `-DryRun`；脚本会真实执行 `SessionStart/UserPromptSubmit/Stop/PreCompact/PostCompact` fixture，并要求 `Stop/PreCompact/PostCompact` 不是 `SKIP`，用于确认压缩前后快照链路真的可用
