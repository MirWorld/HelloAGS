...
补充写入（按需）：
- 若涉及“团队级偏好/长期约束”变化：同步到 `HAGSWorks/project.md` 的“协作与偏好/项目能力画像”
...
### 压缩生命周期检查点（可选，结构化）
- [SRC:TOOL] compact_event: pre_compact # ts=...
- [SRC:TOOL] compact_trigger: auto | manual
- [SRC:TOOL] session_id: ...
- [SRC:TOOL] turn_id: ...
- [SRC:TOOL] model: ...
...
补充（可选字段）：
- `turn_id:` 只在可从 Codex hooks/运行时稳定取得时记录；用途是**把事件绑定到当前 turn**，优先级高于 `trace_id`，可显著降低跨轮误归因
