...
本协议不绑定具体工具形态；只定义“边界与合并规则”。在 Codex 环境里通常有 3 种实现方式：

1. **Fork Thread / Sub-agent（如可用，优先）**
   - 价值：把探索/对照审查隔离在子线程/子代理里，降低主线程被噪声污染、压缩后重做/跑偏的概率。
   - 约束：fork 出去的子线程/子代理仍必须遵守本协议的“只读/无副作用/不扩大范围/证据输出”。

2. **Spawn 多代理（如可用，其次）**
   - 同上：主控唯一可写入；子代理只读侦察/独立审查。
