...
补充（兜底，非默认）：若你已完成本协议恢复（含 No-Redo 判定 + repo_state 双证据）但仍持续“重复提起已完成任务/越界重做”，且你怀疑来自本地记忆污染，可在获得用户确认后执行一次 `codex debug clear-memories`，然后立刻再跑一遍本恢复流程（以磁盘事实重建进度）。该动作不应自动执行。
...
3. **读取任务状态与检查点（找回进度）**
    - 读 `{package}/task.md`：
      ...
      - 补充：断层续作时，Codex 的 `thread/resume` 可能会把“上一轮遗留的等待输入请求/队列输入”恢复出来；一旦界面/系统显示仍在等待输入，也应视为 Pending，先收口到交互等待，不要继续执行/改代码。
...
9. **恢复后 contract 复核（高风险事件后必做）**
   - 当本次恢复触发原因包含以下任一信号时，必须做一次极小 contract 复核：
     ...
     - 明显压缩/截断/模型切换提示
   - 固定检查 4 件事：
     - 当前目标是否仍与 `why.md#对齐摘要` 一致
     - 当前成功标准是否仍成立，且没有被中途替换
     - 非目标 / 禁止项 / 用户追加约束是否仍有效
     - 当前“下一步唯一动作”是否仍合法，且没有绕过 Pending / Delta / 功能删减审批
   - 处理规则：
     - 若 4 项均明确 → 可继续推进
