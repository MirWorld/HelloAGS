...
适用：除纯问答外的所有场景（需要读/写项目文件、扫描 `helloagents/` 工作区、运行项目命令等）。
...
规则：
- 所有 `helloagents/...` 路径（plan/wiki/history/active_context/project 等）都必须以 `PROJECT_ROOT` 为基准定位与写入
- monorepo/多子项目：若用户明确指定“以某子目录为工作区根目录”，以用户指定为准，并写入 `helloagents/project.md#项目能力画像`

停止条件：
- 已得到唯一的 `PROJECT_ROOT`
- 已明确：本轮所有读写都相对 `PROJECT_ROOT` 执行（避免在子目录生成多份 `helloagents/`）
