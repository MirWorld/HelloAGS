...
- 发生模型切换/以不同模型继续/明显 reroute（即使你“感觉还记得”，也更容易产生断层与重复执行）
...
补充（强制收口）：若 Red 的主因是“输出不完整/压缩异常/续作不确定”，默认把“下一步唯一动作”固定为：
- 按 `references/resume-protocol.md` 完整恢复到“可继续执行”的状态（No-Redo 判定 + verify_min 确认）后再继续
...
3. **收敛信息输入**：只读取与下一步动作相关的最小文件片段（优先原生搜索/读取工具；shell `rg` / `Get-Content` / `cat` 仅在不可用、失败或无法表达时 fallback）
4. **把决策写回方案包**：边界/复用/重构预算写 `how.md`；公共接口写 `HAGSWorks/active_context.md`
5. （如适用）**断层恢复**：若出现模型切换/续作不确定/输出不完整，立即按 `references/resume-protocol.md` 重建到“下一步唯一动作”
