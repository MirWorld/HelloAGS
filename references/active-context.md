...
术语口径：见 `references/terminology.md`（校验分层：`loose/strict`）。
...
**每一条 Public API 登记必须包含 `[SRC:CODE]` 指针**，推荐格式：
- `[SRC:CODE] path/to/file.ext symbol`
...
最终输出前（Review 前）必须完成一次“可续作校验”（见第 6 节）。严格校验（strict）为可选增强。
...
## 6) 校验（必须的可续作校验 + 可选严格校验）

目标：保证 active_context 不会“看起来很像真的”但其实早已过时；同时避免因为行号漂移把维护成本抬得过高。
...
适用：
- 仓库重构频繁且希望保持“指针精确”
- 发布/交付前希望更强信号（尤其变更触发了 Public API/契约/数据流承诺）

校验清单（在 6.1 基础上增加）：
- 要求每条 Public API 都提供行号（`:123` 或 `#L123`）
...
- **必须当轮修复（阻断交付）**：本次变更影响 Public API/契约/数据流承诺（见第 4 节触发条件）时，必须当轮修复对应条目的 `[SRC:CODE]` 指针；至少通过 6.1 可续作校验；未修复不得宣称“完成/可交付”
