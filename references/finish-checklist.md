...
- **证据可复现**：fmt/lint/typecheck/test/security 的执行情况与结果可复现记录（命令来自 `helloagents/project.md#项目能力画像`）
...
- **Active Context 可续作**：涉及 Public API/契约变更时，`helloagents/active_context.md` 已更新且每条具备 `[SRC:CODE]`；若指针漂移必须当轮修复（见 `references/active-context.md`）；必要时运行 `helloagents/scripts/validate-active-context.ps1`
