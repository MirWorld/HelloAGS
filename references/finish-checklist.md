...
- **结果目标已覆盖**：用户需求目标、成功标准、非目标已在实现/文档/验证中逐项对上；未覆盖项必须显式标为 `[?]` 或遗留风险
- **关键变更点已说明**：最终输出能说清改了哪些文件、为什么改、影响范围是什么，不用读聊天记录也能复核
...
- **结果结构已对账（命中时）**：若本次交付包含多个产物、多个固定 section、多个页面/模块，已完成结构完整性检查（产物齐全、汇总/明细一致、对象数/结果数可对账）
...
- [ ] 需求目标已覆盖，未偏离用户约束
- [ ] 关键变更点、原因、影响范围已说明
- [ ] 已执行 `verify_min`；更重门禁未运行时已说明原因
- [ ] 无新增明显语法错误、逻辑错误或未处理异常
- [ ] 遗留风险 / 后续项已显式标注
