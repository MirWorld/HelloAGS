...
- 如属于**长任务**（多任务 / 多轮 / 重复触碰同一 Workset / 中后程仍在推进）：除常规 Review 外，必须额外检查“结构侵蚀 / 累计回归 / 相邻变化压力”
...
- 是否存在“看起来完成、但证据不足或只覆盖了局部”的成功标准
- 若 `how.md` 已定义 `carry_forward_verify:`：后置改动是否已重跑这些既有验证，或明确记录“本轮不受影响”
- 非目标是否真的没被“顺手做了”
...
- `HAGSWorks/active_context.md` 是否可续作：Public APIs 是否齐全且每条具备 `[SRC:CODE] path symbol` 指针（行号可选；禁止无来源事实）
- 是否存在未获批准的功能删减、入口删减、默认能力降级、用户可见表面缩减、公开 API/配置/事件缩减（见 `references/feature-removal-guard.md`）
- 若任务经历过压缩 / reroute / incomplete：是否已重新确认当前 contract 仍成立，而不是直接沿用旧结论
...
- **原型墙风险（命中时必查）**：若本轮目标是把 Demo/MVP/原型推向可用，必须检查安全、边缘条件、维护入口、回滚路径、验证路径是否齐全；不能只看“功能能跑”
- **AI 工具链供应链（命中时必查）**：若新增依赖、外部 CLI、MCP、Skill、hook、OAuth/token 或模型代理库，必须按 `references/safety.md` 复核来源、权限、动态执行和回滚方式
- **结果结构完整性（命中时必查）**：若本次任务是多产物 / 多模块 / 多页面 / 固定 section 契约，必须检查产物是否齐全、汇总与明细是否一致、对象数与结果数是否可对账、是否存在后半段缩水

#### 2.2.1 结构侵蚀与后程退化（长任务必查）

目的：识别“任务仍在推进，但代码/结构正在悄悄变差”的状态，避免把长任务做成不断堆补丁。

默认优先检查：
- **热点膨胀**：同一文件 / 函数是否在后续轮次里持续变大，却没有换来更清晰的边界
- **胶水扩散**：是否为了推进速度新增复制逻辑、临时分支、短路路径、一次性 glue code
- **累计回归**：后置任务是否打穿前置成功标准，只验证了“这一步能跑”而没验证“前一步仍成立”
- **相邻变化压力**：当前实现是否明显让下一类合理变化更难做（例如进一步扩展时必须再拆一次 / 再返工一次）
- **权宜实现**：若本轮明确选择了临时路径，是否已在 `task.md##上下文快照` 记录 `design_debt / why_now / revisit_trigger`

#### 2.2.2 止损三问（品味收口，默认推荐）

目的：防止“能做就多做”的范围漂移，把开发从瀑布变成可控的“雕刻”。

- **是否新增了与成功标准无关的抽象/文件？** 若是，优先删减冗余抽象与无关改动面；不得删减用户可见表面、默认能力或既有契约，除非已获得明确批准  
- **是否触碰了非目标/约束边界？** 若是，优先回滚或升级确认（不要硬做）  
- **是否能用更小 diff 达成同等成功标准？** 若能，收敛改动面并用 `verify_min` 重新验收  

#### 2.2.3 反向审查（挑错导向，默认推荐）

目的：避免 Review 退化成“礼貌收尾”，把注意力放在最容易漏掉的缺口上。

默认优先检查：
- **有没有成功标准缺少证据？** 若有，视为未完成而不是“基本完成”
- **有没有结果只是“看起来有”，实际缺 section / 缺对象 / 缺明细？**
- **有没有已完成任务被返工，但没有新增 Delta / 新任务承接？**
- **有没有超出 `why.md#对齐摘要` 的顺手改动，却未在 Review 中解释？**
- **有没有因为压缩 / reroute / incomplete 丢掉旧约束，却仍沿旧路径继续？**
- **有没有“本轮验证是绿的，但结构已经明显恶化”？** 若有，视为未通过 Review，而不是“以后再收拾”
- 若 `how.md` 已写明结果结构目标：Review 必须逐项对照该目标验收，而不是临场发挥
...
产出要求（每条问题必须包含）：
- `[置信度]` + `path`（可选 `:line`）或 `[SRC:CODE]` 指针
...
- 推荐：在 `task.md` 末尾新增 `## Review 记录`，包含：
  ...
  - 采取的修复（≤5条）
  - 复测结果（门禁命令与结果摘要）
  - 归档前必须能追溯到实际验证/复测证据；只有“Review 已完成/验证未执行”这类文字不满足 Archive Readiness Gate

原则：Review 记录必须能解释“为什么这次顺手重构不会影响目标”。
若本次涉及功能删减，Review 记录还必须说明批准记录在哪里，以及替代行为是否与用户批准一致。
默认优先写：遗漏 / 缩水 / 越界 / 返工 / contract 偏移 / 结构侵蚀 / 累计回归 / 设计债务及其修正，不要只复述“做了哪些动作”。
