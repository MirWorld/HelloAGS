...
- 不需要逐条复述清单；只需把关键结论落盘到 `how.md` / `task.md` / `helloagents/active_context.md`
- 栈无关：具体命令来自 `helloagents/project.md#项目能力画像`
...
| 跨层一致性 | 变更涉及 ≥3 层；或改契约/Schema/DTO；或多消费者；或改变错误语义/幂等/鉴权等承诺 | 实现前（必要时实现后复核一次） | `references/cross-layer-checklist.md` | `how.md##API设计/##数据模型` + `helloagents/active_context.md` |
...
- **Public API/契约入口**：写入 `helloagents/active_context.md`（每条必须 `[SRC:CODE] path:line symbol`；禁止无来源事实）
