...
目录: `HAGWroks/plan/YYYYMMDDHHMM_<feature>/`
...
- [ ] 0.5.2 如触发：按 `references/cross-layer-checklist.md` 清点受影响层/契约/消费者；补齐 how.md 的契约与兼容策略；必要时同步更新 `HAGWroks/active_context.md`
...
- [ ] 0.7.1 阅读 `HAGWroks/active_context.md`：把它当作公共接口入口清单（派生层，非 SSOT（真值）），只信带 `[SRC:CODE]` 的 Public APIs 条目
- [ ] 0.7.2 如本次变更影响公共接口/契约/数据流：更新 `HAGWroks/active_context.md`（每条 Public API 必须包含 `[SRC:CODE] path:line symbol`），并在 Review 前完成漂移校验（可选运行 `HAGWroks/scripts/validate-active-context.ps1`）
...
- [ ] 5.4 执行 test（命令来自 `HAGWroks/project.md#项目能力画像`），覆盖 why.md 的成功标准与核心场景
- [ ] 5.5 如涉及依赖/外部输入/权限，执行 security 门禁（命令来自 `HAGWroks/project.md#项目能力画像` 或项目既有检查）

---

## 6. Review（必做）
- [ ] 6.1 Review-规格一致性：对齐摘要（目标/成功标准/非目标/约束）与实现/任务/验证保持一致；`## 上下文快照` 覆盖关键决策/约束/下一步唯一动作且来源标签齐全；`HAGWroks/active_context.md` 可续作且 Public APIs 具备 `[SRC:CODE]` 指针
