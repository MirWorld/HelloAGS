...
| `templates/validate-plan-package.ps1` | `HAGSWorks/scripts/validate-plan-package.ps1` | 方案包完整性校验脚本（plan/exec） |
| `templates/capture-runtime-events.ps1` | `HAGSWorks/scripts/capture-runtime-events.ps1` | 运行时/模型事件回填脚本（无感增强；只写 task 快照） |
