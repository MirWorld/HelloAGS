...
1. 先用原生搜索/读取工具或目录浏览定位你需要的模板文件；原生工具不可用时再用 `rg` fallback
...
| `templates/abandon-plan-package.ps1` | `HAGSWorks/scripts/abandon-plan-package.ps1` | 未执行旧方案的放弃清理脚本（候选过滤后使用） |
...
| `templates/hooks/*-fixture.json` | 本地 dry-run 输入 | hooks 脚本最小行为验证（Stop / SessionStart / UserPromptSubmit / PreCompact / PostCompact） |
| `scripts/check-target-hooks.ps1` | 目标项目只读诊断命令 | 检查 `.codex/config.toml`、`.codex/hooks.json` 与 helloagents hook 脚本路径；`-DryRun` 可真实执行 fixture 验证 active 方案包下的 hooks |
