...
> 默认填写预算：标准方案也先写“最小默认 + 触发扩展”。未命中触发条件的小节只保留 1 行结论，不展开清单；Quick Fix 任务应改用 quickfix 模板。

## 技术方案
- **核心改动:** [用 1–3 条说清这次真正要改什么]
- **代码落点:** [文件/模块/入口]
- **verify_min:** `...`
- **carry_forward_verify:** `无`（若后续任务还会再次触碰同一 Workset，列出需持续回归的既有验证）
...
- **待确认:** [需要用户/环境确认的点]
- **替代路径:** [如假设不成立时的备选方案]
- **收缩规则:** 未影响方案选择的假设不写；不要为了填满模板制造待确认项
...
## 相邻变化压力（命中时最小展开）
- **默认写法:** 当前实现不应明显阻塞最邻近的后续变化；未命中可写“无额外压力”
- **需要展开时:** [下一类合理变化是什么 / 当前实现应避免把哪一步堵死]
...
- **累计回归（如适用）:** `carry_forward_verify: ...`（后续任务再次触碰同一 Workset 时需要顺带重跑的既有验证）
...
- 若 hooks / 快照 / `_current.md` 已提供结构事实，how.md 只引用结果，不重复解释同一守卫逻辑
...
- 若本次交付存在固定结构契约（多产物 / 多 section / 多页面 / 多模块），在此补充最小结构目标与验收点：
  - 应有几类产物
  - 每类至少包含什么
  - 哪些数量关系需要对账（例如“产物数一致 / section 齐全 / 汇总与明细可对账”）
- **测试:** [策略]
- **部署:** [流程]
