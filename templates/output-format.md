...
**3. 验证步骤** - 输出前MUST自检:
   ...
   [ ] 确认包含状态符号(✅/❓/⚠️/🚫/❌)
   [ ] 确认文件清单使用纵向列表
   [ ] 若检测到输出不完整/压缩或续作异常（例如 `response.incomplete`），不得输出“完成类总结”；改用G6.2异常格式并优先执行断层恢复（`references/resume-protocol.md`）
   ```
...
遗留方案清单:
📦 遗留方案: 检测到 X 个未执行且非 active 的方案包（已过滤当前包、已执行/半执行包）:
  - {方案包名称1}
  - {方案包名称2}
  ...
是否需要迁移至历史记录?（仅限用户明确放弃续作的未执行旧方案；已执行/半执行方案必须先恢复续作或通过 Archive Readiness Gate）
...
检测到输出不完整/压缩或续作异常（例如 `response.incomplete`），无法可靠判断当前进度，已暂停以避免重复修改或越界。

────
🔄 下一步: 按 `references/resume-protocol.md` 执行断层恢复（重建到“下一步唯一动作”）
```
