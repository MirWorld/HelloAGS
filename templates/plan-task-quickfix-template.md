...
- `### 功能删减审批`、`### 错误与尝试`、`## Active Context 更新记录` 仅在命中时填写；未命中保持占位即可
- `## Review 记录` 是收尾必填项；极小任务也至少写 1 条“Review 结论 + 验证/复测证据”，否则不得归档
...
- [ ] 2.4 归档就绪检查：仅当所有任务已终态、Pending 为空、`progress_phase: final`、Review 记录已填写且验证证据可追溯时，才允许按 `references/plan-lifecycle.md` 迁移到 history；否则保持当前方案包 active 并写清下一步唯一动作
...
解析兼容：`[✓]` 等同 `[√]`，`[x]` 等同 `[X]`；写入时优先使用 canonical 符号 `[√]` / `[X]`。
